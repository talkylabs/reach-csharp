...
using Reach.Http;
using HttpMethod = Reach.Http.HttpMethod;
...
namespace Reach.Tests.Http
{
    ...
    [TestFixture]
    public class SystemNetHttpClientTest : ReachTest
    {
        private MockResponseHandler _mockHttp;
        public SystemNetHttpClient ReachHttpClient { get; set; }
        ...
        public void Init()
        {
            ...
            this.ReachHttpClient = new SystemNetHttpClient(internalHttpClient);
        }
        ...
        public void TestMakeRequestSuccess()
        {
            this._mockHttp.Respond(
                "https://api.reach.talkylabs.com/v1/Resource.json",
                HttpStatusCode.OK,
                "{'test': 'val'}"
            );

            Request testRequest = new Request(HttpMethod.Get, "https://api.reach.talkylabs.com/v1/Resource.json");
            Response resp = this.ReachHttpClient.MakeRequest(testRequest);

            Assert.AreEqual(HttpStatusCode.OK, resp.StatusCode);
            Assert.AreEqual("{'test': 'val'}", resp.Content);

            Assert.AreSame(testRequest, this.ReachHttpClient.LastRequest);
            Assert.AreSame(resp, this.ReachHttpClient.LastResponse);
        }
        ...
        public void TestMakeRequestAsyncSuccess()
        {
            ...
            Request testRequest = new Request(HttpMethod.Get, "https://api.reach.talkylabs.com/v1/Resource.json");
            Task<Response> result = this.ReachHttpClient.MakeRequestAsync(testRequest);
            result.Wait();
        ...
        public void TestMakeRequestAsyncSuccessThreadSafe()
        {
            ...
            {
                this._mockHttp.Respond(
                    "https://api.reach.talkylabs.com/v1/" + i + "/Resource.json",
                    HttpStatusCode.OK,
                    "{'test': 'val" + i + "'}"
                );
            }
            ...
            {
                int i = (int)index;
                Request testRequest = new Request(HttpMethod.Get, "https://api.reach.talkylabs.com/v1/" + i + "/Resource.json");
                Task<Response> result = this.ReachHttpClient.MakeRequestAsync(testRequest);
                result.Wait();
        ...
        {
            this._mockHttp.Respond("https://api.reach.talkylabs.com/v1/Resource.json", HttpStatusCode.InternalServerError);
        ...
        public void TestMakeRequestThrowsOnConnectionErrors()
        {
            ...
            Assert.Throws<HttpRequestException>(() => ReachHttpClient.MakeRequest(testRequest));

            Assert.AreSame(testRequest, this.ReachHttpClient.LastRequest);
            Assert.IsNull(this.ReachHttpClient.LastResponse);
        }

        [Test]
        public void TestMakeRequestWithParams()
        {
            this._mockHttp.Respond("https://api.reach.talkylabs.com/v1/Resource.json", HttpStatusCode.OK);

            Request testRequest = new Request(HttpMethod.Post, "https://api.reach.talkylabs.com/v1/Resource.json");
            testRequest.AddPostParam("post_param", "post_value");
            testRequest.AddQueryParam("query_param", "query_value");

            this.ReachHttpClient.MakeRequest(testRequest);

            HttpRequestMessage internalRequest = this._mockHttp.InternalRequest;

            Assert.AreEqual("https://api.reach.talkylabs.com/v1/Resource.json?query_param=query_value",
        ...
        public void TestMakeRequestAddsHeadersAndUserAgent()
        {
            ...
            Assert.AreEqual("username", internalRequest.Headers.GetValues("ApiUser").ElementAt(0));
            Assert.AreEqual("password", internalRequest.Headers.GetValues("ApiKey").ElementAt(0));
            ...
            Regex rgx = new Regex(@"^reach-csharp/[0-9.]+(-rc\.[0-9]+)?\s\(\w+\s\w+\)\s[.\s\w]+/[^\s]+$");
