...
{
    [TestFixture]
    public class WebRequestClientTest : ReachTest
    {
        public WebRequestClient ReachClient;
        ...
        public void Init()
        {
            ...
            this.ReachClient = new WebRequestClient(this._mockFactory);
        }
        ...
        public void TestMakeRequestSuccess()
        {
            ...
            // Run Test
            var request = new Request(HttpMethod.Get, "https://api.reach.talkylabs.com/v1/Resource.json");
            var response = this.ReachClient.MakeRequest(request);

            // Assert
            Assert.AreEqual(new Uri("https://api.reach.talkylabs.com/v1/Resource.json"), this._requestUrl);

            Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
            Assert.AreEqual("{'key': 'val'}", response.Content);
            Assert.IsNull(this._requestBody);

            Assert.AreSame(request, this.ReachClient.LastRequest);
            Assert.AreSame(response, this.ReachClient.LastResponse);
        }
        ...
        public void TestMakeRequestThrowsOnConnectionError()
        {
            ...
            try
            {
                this.ReachClient.MakeRequest(request);
            ...
            Assert.IsNull(this._requestBody);
            Assert.AreSame(request, this.ReachClient.LastRequest);
            Assert.IsNull(this.ReachClient.LastResponse);
        }
        ...
        public void TestMakeRequestWithParams()
        {
            ...
            // Run Test
            var request = new Request(HttpMethod.Post, "https://api.reach.talkylabs.com/v1/Resource.json");
            ...
            // Assert
            Assert.AreEqual(new Uri("https://api.reach.talkylabs.com/v1/Resource.json?query_param=query_value"), this._requestUrl);
        ...
        public void TestMakeRequestAddsHeadersAndUserAgent()
        {
            ...
            Regex rgx = new Regex(@"^reach-csharp/[0-9.]+(-rc\.[0-9]+)?\s\(\w+\s\w+\)\s.NET/3.5$");
