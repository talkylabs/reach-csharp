...
namespace Reach.Base
{
    ...
    public class ResourceSet<T> : IEnumerable<T> where T : Resource
    {
        ...
        private readonly IReachRestClient _client;
        ...
        public ResourceSet(Page<T> page, ReadOptions<T> options, IReachRestClient client)
        {
            _page = page;
            _options = options;
            _client = client;
            ...
            {
                _pageLimit = (long) (Math.Ceiling((double) _options.Limit.Value / _options.PageSize.Value));
            }
        }
        ...
        {
#if !NET35
            return typeof(T).GetRuntimeMethod("NextPage", new[]{ typeof(Page<T>), typeof(IReachRestClient) });
#else
            return typeof(T).GetMethod("NextPage", new[]{ typeof(Page<T>), typeof(IReachRestClient) });
#endif
        }
    }
}
