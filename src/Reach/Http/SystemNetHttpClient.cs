...
namespace Reach.Http
{
    ...
    public class SystemNetHttpClient : HttpClient
    {
        ...
        /// </summary>
        /// <param name="request">Reach response</param>
        ...
        private HttpRequestMessage BuildHttpRequest(Request request)
        {
            ...
            httpRequest.Headers.Add(apiUserHeader, request.Username);
            httpRequest.Headers.Add(apiKeyHeader, request.Password);
...
#endif
            var libraryVersion = String.Format("reach-csharp/{0} ({1} {2}) {3}", helperLibVersion, osName, osArch, PlatVersionSb);
